  ...
  return (
    <div className="min-h-[100dvh] bg-ink">
      ...
      <section className="relative min-h-[72vh] overflow-hidden pt-28">
        ...
        <div className="relative flex min-h-[72vh] flex-col justify-end px-5 pb-16 sm:px-10 lg:px-16">
          <p className="font-mono text-[10px] tracking-[0.42em] text-brass uppercase">
            {category.index}  ·  {category.latin}
          </p>
          <h1 className="mt-4 max-w-3xl font-serif text-5xl leading-[0.92] text-ivory sm:text-7xl">
            {category.headline}
          </h1>
      ...
      {category.id === "software" ? (
        <section className="border-y border-ivory/10 bg-ivory/[0.03] px-5 py-12 sm:px-10 lg:px-16">
          ...
          <h2 className="mt-3 max-w-2xl font-serif text-4xl text-ivory">
      ...
      <section className="px-5 py-16 sm:px-10 lg:px-16">
        ...
        <div className="showroom-rail flex gap-6 overflow-x-auto pb-8 md:grid md:grid-cols-2 md:overflow-visible lg:grid-cols-3">
          {items.map((product) => (
            <Link
              key={product.slug}
              href={`/exhibit/${product.slug}`}
              className="group min-w-[78vw] md:min-w-0"
            >
              <div className="exhibit-hero relative aspect-[4/5] overflow-hidden">
                ...
                />
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-ink via-ink/40 to-transparent p-5">
                  <p className="font-mono text-[10px] tracking-[0.28em] text-brass uppercase">
                    {product.status}  ·  {product.kind}
                  </p>
                  <h3 className="font-serif text-3xl leading-none text-ivory">
                    {product.name}
                  </h3>
